...
void drawProgressBar(int songProgress, int songDuration, int y = 30, int height = 7) {
  ...
  // Draw background bar outline (rounded edges)
  display.drawRoundRect(0, y, SCREEN_WIDTH, height, radius, WHITE);

  // Draw filled rounded bar
  if (barWidth > radius * 2) {
    ...
    // If bar is very small, just draw a filled circle
...
void drawPlayPauseIcon(bool isPaused, bool isMuted, int y = 56) {
  ...
  // === LEFT ICON: Improved Mute / Unmute ===
  ...
  // === CENTER ICON: Play / Pause (unchanged) ===
  if (isPaused) {
  ...
  // === RIGHT ICON: Volume (optional placeholder) ===
